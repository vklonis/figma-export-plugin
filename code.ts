...
figma.ui.onmessage = async (msg) => {
  ...
  if (msg.type === "export-frames") {
    try {
      const frameIds = msg.frameIds as string[];
...
async function processNodeRecursive(
  node: SceneNode,
  rootFrame: FrameNode,
  outputNodes: ExportNode[],
  ...
  const isRootFrame = node === (rootFrame as SceneNode);
  const relativeX = isRootFrame ? 0 : ("x" in node ? (node as any).x as number : 0);
  const relativeY = isRootFrame ? 0 : ("y" in node ? (node as any).y as number : 0);
  ...
  switch (node.type) {
    case "RECTANGLE":
    case "ELLIPSE":
      await processShapeNode(
        node as RectangleNode | EllipseNode,
        exportNode,
        images,
      );
      break;
    ...
    case "TEXT":
      await processTextNode(node as TextNode, exportNode);
      break;
    ...
    case "BOOLEAN_OPERATION":
      ...
      await exportAsImage(node as SceneNode, exportNode, images);
      break;
    ...
    case "COMPONENT":
      ...
      if ("clipsContent" in node) {
        exportNode.clipsContent = (node as FrameNode).clipsContent;
      } else {
...
async function processShapeNode(
  ...
  if (
    ...
    exportNode.cornerRadius = node.cornerRadius as number | number[];
  }
...
async function processTextNode(node: TextNode, exportNode: ExportNode) {
  ...
  if (node.fills !== figma.mixed) {
    exportNode.fills = await processPaints(node.fills as Paint[], {});
  }
}
...
function extractBackgroundColor(frame: FrameNode): string {
...
function getFontWeight(fontName: FontName | symbol): number {
  if (typeof fontName === "symbol") return 400;
